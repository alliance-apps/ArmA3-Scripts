...
if(_sel isEqualTo 1) exitWIth { //name
    _query = format["UPDATE gangs SET name='%1' WHERE id='%2'",[_group getVariable ["gang_name",""]] call DB_fnc_mresString,_id];
    [_query,1] call DB_fnc_asyncCall;
};
...
if(_sel isEqualTO 6) exitWith { //permission
    _query = format["UPDATE gangs SET permission='%1',perm_int='%2' WHERE id='%3'",[_group getVariable ["gang_permissions",[]]] call DB_fnc_mresArray,_group getVariable ["gang_perm_int",1],_id];
    [_query,1] call DB_fnc_asyncCall;
};
...
if(_sel isEqualTo 11) exitWIth { //memberS
    ...
    if(_arr isEqualTo []) exitWith {};
    {
        ...
        [10,_group,(_x select 0),_gang_info] call ton_fnc_gang_update;
    } forEach _arr;
};
...
if(_sel isEqualTo 13) exitWith { //hq
    ...
    _query = format["UPDATE gangs SET hq='%1', hq_upgrades='%2', hq_garage='%3' WHERE id='%4'",_group getVariable ["gang_hq_id",-1],[_group getVariable ["gang_hq_upgrades",[]]] call DB_fnc_mresArray,_group getVariable ["gang_garage_info",[]],_id];
	[_query,1] call DB_fnc_asyncCall;
	if(_bool) then {
